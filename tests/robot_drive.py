...
class RobotDrive:
    def __init__(self, port="/dev/ttyTHS1", baud=115200, inter_cmd_delay=0.03):
        self.client = Esp32WheelClient(port, baud)
    ...
    def _send_wheels(self, fl, rl, fr, rr):
        ok = True
        ok &= self.client.set_wheel_speed("fl", fl)
        time.sleep(self.inter_cmd_delay)
        ok &= self.client.set_wheel_speed("rl", rl)
        time.sleep(self.inter_cmd_delay)
        ok &= self.client.set_wheel_speed("fr", fr)
        time.sleep(self.inter_cmd_delay)
        ok &= self.client.set_wheel_speed("rr", rr)
        return ok
